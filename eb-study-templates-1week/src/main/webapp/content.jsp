...
<%--//        Statement--%>
<%--        PreparedStatement pstPost = conn.prepareStatement("SELECT * FROM tb_board_list WHERE board_num=?");--%>
<%--        pstPost.setInt(1, boardnum);--%>

<%--        rsPost = pstPost.executeQuery();--%>
<%--        if (rsPost.next()) {--%>
<%--            post.name = xxx;--%>
<%--            post.contents  = xxxxxx;--%>
<%--        }--%>
<%--    }catch (Exception e){--%>
<%--        //TODO:  somethning..--%>
<%--    }finally {--%>
...
<%
    ...
    try {
        ...
        PreparedStatement pstPost = conn.prepareStatement("SELECT * FROM board_list WHERE boarnum=?");
        pstPost.setInt(1, boardnum);
